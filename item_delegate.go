...
func (d itemDelegate) Render(w io.Writer, m list.Model, index int, item list.Item) {
	...
	modifiedStyle := styles.ItemIDStyle() // Using same style as ID for consistency

	if index == m.Index() {
		// Apply border and highlight styles for selected item
		nameStyle = nameStyle.Bold(true).BorderLeft(true).BorderStyle(lipgloss.InnerHalfBlockBorder()).BorderForeground(styles.GetTheme().GetColor(styles.GetTheme().Colors.ItemBorder)).PaddingLeft(1)
		...
		idStyle = idStyle.Bold(true).BorderLeft(true).PaddingLeft(-2).PaddingRight(-2)
	}
	...
	modified := modifiedStyle.Width(modifiedWidth).Render(fmt.Sprintf("%-*s", modifiedWidth-padding, model.Modified.Format("2006-01-02")))
	id := idStyle.Width(idWidth).Render(fmt.Sprintf("%-*s", idWidth-padding, model.ID))
